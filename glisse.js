 ...
 */ (function ($) {
    $.glisse = function (element, options) {
        ...
        var plugin = this,
            ...
            // Private var
            pictureUrl, group, isChange = false,
            touch = {},
            cache = [];
        ...
        var createElements = function createElements() {
                ...
                plugin.els['content'] = $(document.createElement('div')).attr('id', 'glisse-overlay-content').css(cssProp, cssVal).css(getPrefix('transform') + 'transform', 'scale(0)');
                plugin.els['controls'] = $(document.createElement('div')).attr('id', 'glisse-controls').css(cssProp, cssVal);
                plugin.els['controlNext'] = $(document.createElement('span')).attr('class', 'glisse-next').append($(document.createElement('a')).html('&#62;').attr("href", "#"));
                plugin.els['controlLegend'] = $(document.createElement('span')).attr('class', 'glisse-legend');
                plugin.els['controlPrev'] = $(document.createElement('span')).attr('class', 'glisse-prev').append($(document.createElement('a')).html('&#60;').attr("href", "#"));

                // Add structure
                plugin.els['overlay'].append(plugin.els['spinner']);
                plugin.els['controls'].append(
                plugin.els['controlNext'], plugin.els['controlLegend'], plugin.els['controlPrev']);
                plugin.els['wrapper'].append(
                plugin.els['overlay'], plugin.els['close'], plugin.els['content'], plugin.els['controls']);
        ...
        var changePicture = function changePicture(direction) {
                ...
                if (change && isChange === false) {
                    ...
                    if (cache.indexOf(pictureUrl) === -1) spinner(true);
                    ...
                    setTimeout(function () {
                        if (plugin.settings.mobile) {
                            plugin.els['content'].css(getPrefix('transform') + 'transform', 'translateX(0px)').css('display', 'none');
                        }
                        ...
                        img.load(function () {
                            ...
                            if (cache.indexOf(pictureUrl) === -1) spinner(false);

                            if (plugin.settings.mobile) {
                                plugin.els['content'].css('display', 'block');
                            }
                            plugin.els['content'].removeClass('glisse-transitionOut-' + direction).addClass('glisse-transitionIn-' + direction);
                            setTimeout(function () {
        ...
        var setChangeStatus = function setChangeStatus() {
                var $currentEl = $('img[data-glisse-big="' + pictureUrl + '"]'),
                    parent = plugin.settings.parent ? $currentEl.closest(plugin.settings.parent) : $currentEl.parent();
        ...
        var getPrefix = function getPrefix(prop) {
                ...
                for (var len = prefixes.length; len--;) {
                    if ((prefixes[len] + upper) in elem.style) return ('-' + prefixes[len].toLowerCase() + '-');
                }
        ...
        var isValidEffect = function isValidEffect(effect) {
                ...
                if (typeof (effect) == 'string' && isNaN(effect) && fx.indexOf(effect) !== -1) return true;
            };
